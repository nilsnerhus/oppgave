...
#' overall_dominance <- find_dissonance(topic_data, value_col = "Proportion")
#' 
#' # Calculate dominance for Africa region
#' africa_dominance <- find_dissonance(
#'   topic_data, 
...
find_dissonance <- function(
  ...
  ## --- Input validation -----------------------------------------------------
  tryCatch({
    validate_input(data, c(value_col), "find_dissonance")
    ...
    log_message(paste("Validation error:", e$message), "find_dissonance", "ERROR")
  ...
  result <- time_operation({
    ...
    if (length(values) == 0) {
      log_message("No valid values found", "find_dissonance", "WARNING")
    ...
    total <- sum(values)
    if (total <= 0) {
      log_message("Sum of values is zero or negative", "find_dissonance", "WARNING")
    ...
    if (n_actual < n) {
      log_message(paste("Using", n_actual, "values instead of requested", n,
                        "(not enough data)"), "find_dissonance", "INFO")
    }
