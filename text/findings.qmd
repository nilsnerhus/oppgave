...
# Topic reference function
topic_ref <- function(id) {
  name <- topics$data$topics_table$topic_name[topics$data$topics_table$topic_id == id]
  paste0(name, " (Topic ", id, ")")
}

# Extract key model info
topics_table <- topics$data$topics_table
k_metrics <- k_result$data$metrics
k_val <- k_result$data$best_k

# Calculate topic prevalence and coverage
...
topic_prevalence <- data.frame(
  topic_id = 1:k_val,
  topic_name = topics_table$topic_name,
  prevalence_pct = round(topic_props * 100, 1),
  doc_coverage = docs_per_topic,
  coverage_pct = round((docs_per_topic / nrow(model$data$topic_proportions)) * 100, 1)
) |>
  dplyr::arrange(desc(prevalence_pct))

# Key statistics for inline reference (store as decimals)
sem_coh <- k_metrics$Coherence[k_metrics$K == k_val]
exclus <- k_metrics$Exclusivity[k_metrics$K == k_val]
opt_score <- k_metrics$Score[k_metrics$K == k_val]

# Topic rankings
top_5_topics <- paste(topic_prevalence$topic_name[1:5], collapse = ", ")
bottom_3_topics <- paste(topic_prevalence$topic_name[(k_val-2):k_val], collapse = ", ")
top_5_prev <- sum(topic_props[order(topic_props, decreasing = TRUE)[1:5]])
bottom_3_prev <- sum(topic_props[order(topic_props, decreasing = FALSE)[1:3]])

# Document engagement stats
avg_topics_10pct <- mean(apply(model$data$topic_proportions > 0.10, 1, sum))
avg_topics_1pct <- mean(apply(model$data$topic_proportions > 0.01, 1, sum))
universal_topics <- sum(docs_per_topic/nrow(model$data$topic_proportions) > 0.90)
specialized_topics <- sum(docs_per_topic/nrow(model$data$topic_proportions) < 0.50)
```
...
The selected model with k=`r num(k_val)` achieved a semantic coherence score of `r num(sem_coh)` and an exclusivity score of `r num(exclus)`, representing the best balance among tested specifications. This model's coherence score indicates that the most probable words within each topic genuinely appear together in documents rather than being statistical artifacts. The exclusivity score confirms that topics capture distinct aspects of adaptation discourse rather than variations on the same theme. The combined optimization score of `r num(opt_score)` exceeded all other specifications, providing quantitative support for this selection. Beyond these metrics, the `r num(k_val)`-topic model produced themes that aligned with established categories in adaptation planning while revealing nuanced distinctions that might inform policy analysis.

```{r topic_table}
# Display topic prevalence table
knitr::kable(
  topic_prevalence,
  col.names = c("ID", "Topic Name", "Prevalence (%)", "Documents", "Coverage (%)"),
  caption = "Topic Prevalence Across All National Adaptation Plans"
)
```
...
Examining which topics appear most and least frequently across the corpus provides insight into the priorities and gaps in global adaptation planning. The five most prevalent topics—`r top_5_topics`—collectively account for `r pct(top_5_prev)` of the total discourse. The dominance of financial and implementation topics among the most prevalent themes suggests that "how to adapt" receives as much or more attention than "what adaptation looks like." This emphasis on process and mechanisms over substantive actions may reflect the influence of international guidelines and donor requirements in shaping NAP development.

Conversely, the least prevalent topics—`r bottom_3_topics`—collectively represent only `r pct(bottom_3_prev)` of discourse. The marginal presence of topics related to indigenous knowledge, transformational change, or justice-oriented approaches suggests that alternative framings of adaptation remain peripheral to mainstream planning despite their prominence in academic and civil society discussions. This absence is particularly notable given the theoretical literature on adaptation that emphasizes the importance of local knowledge and transformative approaches.

The distribution of topics across documents reveals additional patterns in how adaptation discourse is structured. On average, each NAP substantially engages with `r num(avg_topics_10pct)` topics (using a 10% threshold for substantial engagement), while touching on `r num(avg_topics_1pct)` topics to some degree. This concentration suggests that most countries focus their adaptation planning on a core set of concerns while giving cursory attention to others. The most "universal" topics—those appearing in over 90% of documents—are `r num(universal_topics)` in number, primarily covering financial mechanisms and general planning processes. In contrast, `r num(specialized_topics)` topics appear in fewer than half of all documents, indicating specialized concerns relevant only to specific contexts.
...
# Overall dominance (already 0-1 scale)
overall_doc_dom <- doc_level[doc_level$category == "Overall", "normalized_dominance"]
overall_corpus_dom <- corpus_level[corpus_level$category == "Overall", "normalized_dominance"]

# Income-based dominance (document level)
income_doc <- doc_level[doc_level$category == "Income", ]
low_inc_doc_dom <- income_doc$normalized_dominance[income_doc$subcategory == "Low income"]
high_inc_doc_dom <- income_doc$normalized_dominance[income_doc$subcategory == "High income"]
doc_inc_diff <- abs(high_inc_doc_dom - low_inc_doc_dom)

# Income-based dominance (corpus level)
income_corpus <- corpus_level[corpus_level$category == "Income", ]
high_inc_corpus_dom <- income_corpus$normalized_dominance[income_corpus$subcategory == "High income"]
low_inc_corpus_dom <- income_corpus$normalized_dominance[income_corpus$subcategory == "Low income"]
lower_mid_corpus_dom <- income_corpus$normalized_dominance[income_corpus$subcategory == "Lower middle income"]
upper_mid_corpus_dom <- income_corpus$normalized_dominance[income_corpus$subcategory == "Upper middle income"]

# Income topic focuses
high_inc_topics <- income_corpus$top_topics[income_corpus$subcategory == "High income"]
low_inc_topics <- income_corpus$top_topics[income_corpus$subcategory == "Low income"]
lower_mid_topics <- income_corpus$top_topics[income_corpus$subcategory == "Lower middle income"]
upper_mid_topics <- income_corpus$top_topics[income_corpus$subcategory == "Upper middle income"]

# Regional patterns
region_corpus <- corpus_level[corpus_level$category == "Region", ]
region_doc <- doc_level[doc_level$category == "Region", ]
max_region_dom <- max(region_corpus$normalized_dominance)
min_region_dom <- min(region_corpus$normalized_dominance)
region_dom_range <- max_region_dom - min_region_dom
max_region_name <- region_corpus$subcategory[which.max(region_corpus$normalized_dominance)]
min_region_name <- region_corpus$subcategory[which.min(region_corpus$normalized_dominance)]
max_region_topics <- region_corpus$top_topics[which.max(region_corpus$normalized_dominance)]
min_region_topics <- region_corpus$top_topics[which.min(region_corpus$normalized_dominance)]

# Geographic patterns  
geo_corpus <- corpus_level[corpus_level$category == "Geography", ]
sids_corpus_dom <- geo_corpus$normalized_dominance[geo_corpus$subcategory == "SIDS"]
lldc_corpus_dom <- geo_corpus$normalized_dominance[geo_corpus$subcategory == "LLDC"]
sids_topics <- geo_corpus$top_topics[geo_corpus$subcategory == "SIDS"]
lldc_topics <- geo_corpus$top_topics[geo_corpus$subcategory == "LLDC"]

# Variance patterns (already raw values)
high_inc_var <- income_corpus$variance[income_corpus$subcategory == "High income"]
low_inc_var <- income_corpus$variance[income_corpus$subcategory == "Low income"]
```
...
The document-level findings present a striking picture of universal concentration. Across all 45 NAPs, the normalized dominance value is `r pct(overall_doc_dom)`, indicating that the typical adaptation plan devotes the vast majority of its content to just three topics out of the `r num(k_val)` identified. This pattern holds with remarkable consistency across different country groupings. Low-income countries show a document-level dominance of `r pct(low_inc_doc_dom)`, while high-income countries register `r pct(high_inc_doc_dom)`—a difference of less than `r num(doc_inc_diff)` despite vastly different resources and institutional capacities. The uniformity extends across regional groupings, with values ranging only from `r pct(min_region_dom)` to `r pct(max_region_dom)`, and geographic categories showing similarly minimal variation.
...
The corpus-level patterns reveal more nuanced differences in how countries converge or diverge in their adaptation priorities. Among income groups, a surprising U-shaped pattern emerges that challenges linear assumptions about development and discourse diversity. High-income countries show the highest corpus-level dominance at `r pct(high_inc_corpus_dom)`, indicating strong convergence on similar topics—specifically `r high_inc_topics`. However, with only four high-income countries in the sample (Israel, Kuwait, Trinidad and Tobago, and Uruguay), this finding must be interpreted cautiously. These countries span different regions and face different climate challenges, yet their adaptation discourse converges remarkably, potentially reflecting shared access to international expertise or similar institutional models.

```{r dominance_plot}
# Prepare clean data
dom_plot_data <- rbind(
  income_corpus[income_corpus$subcategory != "High income", 
                c("category", "subcategory", "normalized_dominance")],
  region_corpus[, c("category", "subcategory", "normalized_dominance")],
  geo_corpus[, c("category", "subcategory", "normalized_dominance")]
)
dom_plot_data$dominance_pct <- dom_plot_data$normalized_dominance * 100

dominance_plot <- ggplot2::ggplot(dom_plot_data, 
                        ggplot2::aes(x = dominance_pct, 
                            y = reorder(subcategory, dominance_pct),
                            fill = category)) +
  ...
  ggplot2::labs(title = "Discourse centralization by country groupings",
       x = "Dominance index (%)",
       y = NULL) +
  ...
  ggplot2::theme(
    ...
    legend.title = ggplot2::element_text(size = 10, color = "#495057"),
    legend.text = ggplot2::element_text(size = 9, color = "#495057"),
...
Low-income countries display the second-highest corpus-level dominance at `r pct(low_inc_corpus_dom)`, with discourse concentrated on `r low_inc_topics`. This high centralization among low-income countries aligns with theoretical expectations about the constraining effects of international development frameworks on adaptation planning. The dominant topics emphasize process-oriented and finance-related themes, suggesting that low-income countries' adaptation discourse is significantly shaped by donor requirements and international guidelines. The limited epistemological space reflected in this high dominance value indicates that despite facing diverse climate challenges—from drought in African nations to sea-level rise in Pacific islands—low-income countries articulate adaptation through remarkably similar framings.

The middle-income countries present a markedly different pattern, with lower-middle income countries showing the lowest corpus-level dominance at `r pct(lower_mid_corpus_dom)` and upper-middle income countries at `r pct(upper_mid_corpus_dom)`. This greater diversity in middle-income countries' adaptation discourse suggests more epistemological space for varied approaches. Lower-middle income countries emphasize `r lower_mid_topics`, while upper-middle income countries focus on `r upper_mid_topics`. The different dominant topics and lower overall concentration indicate that middle-income countries may have greater autonomy in framing adaptation according to national priorities rather than international templates. This could reflect their intermediate position—less dependent on climate finance than low-income countries but not yet fully integrated into high-income knowledge networks.

Regional patterns in corpus-level dominance provide additional insight into the factors shaping adaptation discourse. The highest regional concentration appears in `r max_region_name` at `r pct(max_region_dom)`, where countries converge strongly on topics related to `r max_region_topics`. This high concentration may reflect shared regional institutions, similar colonial histories, or common climate challenges. In contrast, `r min_region_name` shows the lowest regional dominance at `r pct(min_region_dom)`, with more diverse emphasis across topics including `r min_region_topics`. This regional variation suggests that epistemic communities operating at the regional scale—through bodies like regional development banks, economic communities, or technical cooperation networks—play a significant role in shaping how adaptation is conceptualized.
...
Geographic vulnerability categories reveal yet another pattern in discourse centralization. Small Island Developing States show a corpus-level dominance of `r pct(sids_corpus_dom)`, with convergence around `r sids_topics`. The presence of Pacific-specific and ocean-related topics among SIDS' dominant themes appears logical given their shared vulnerabilities to sea-level rise and ocean changes. Landlocked Developing Countries display lower dominance at `r pct(lldc_corpus_dom)`, emphasizing `r lldc_topics`. The relatively low dominance values for both geographic categories—lower than most income and regional groupings—suggests that geographic vulnerability alone does not produce convergent adaptation discourse. Instead, countries facing similar physical challenges may still conceptualize adaptation differently based on other factors.

The variance patterns within each grouping provide additional insight into discourse dynamics. Document-level variance is consistently low across all categories, reinforcing the finding of universal concentration in individual NAPs. However, corpus-level variance tells a different story. High-income countries show the highest variance (`r num(high_inc_var)`), suggesting that despite high average concentration, there is considerable variation among these four countries—possibly reflecting their different regional contexts and climate challenges. Low-income countries show much lower variance (`r num(low_inc_var)`), indicating more uniform convergence on similar topics, potentially due to common constraints and dependencies on international frameworks.

Examining which specific topics dominate across different groupings reveals the substantive content of discourse convergence. Financial and procedural topics appear among the top three for multiple groupings, suggesting these themes form a common core of adaptation discourse globally. However, the specific combinations vary meaningfully. Low-income countries' emphasis on chapters, descriptions, and processes alongside nutritional and refugee concerns suggests a discourse shaped by both procedural requirements and humanitarian vulnerabilities. High-income countries' focus on marine and other themes indicates a different framing of adaptation. Middle-income countries show more varied patterns, with some emphasizing provincial and municipal implementation while others focus on sectoral themes like water and agriculture.
