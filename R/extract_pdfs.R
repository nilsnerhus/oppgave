...
extract_pdfs <- function(
	...
	if (length(missing_cols) > 0) {
		error_msg <- paste(
			"tokens_data is missing required columns:",
	...
	if (!dir.exists(pdf_dir)) {
		...
		log_message(
			paste("Created directory:", pdf_dir),
			"extract_pdfs"
		)
	}
	...
	log_message(
		paste("Processing", nrow(tokens_data), "NAP documents"),
		"extract_pdfs"
	)
	...
	for (i in 1:nrow(tokens_data)) {
		...
		log_message(
			paste(i, "of", nrow(tokens_data), ":", doc_id),
			"extract_pdfs"
		)
		...
		tryCatch(
			{
				...
				if (
					...
					diagnostics$download_errors <- c(
						diagnostics$download_errors,
						error_msg
					)
					log_message(
						error_msg,
		...
		if (download_success) {
			tryCatch(
				...
				error = function(e) {
					error_msg <- paste(
						"Error extracting text from PDF for",
						doc_id,
						":",
						e$message
					)
					diagnostics$extraction_errors <- c(
						diagnostics$extraction_errors,
						error_msg
					)
					log_message(
						paste(
	...
	## --- Report errors ----------------------------------------------------------
	total_errors <- length(diagnostics$download_errors) +
		length(diagnostics$extraction_errors)

	if (total_errors > 0) {
		log_message(
			paste(total_errors, "errors encountered:"),
			"extract_pdfs",
			"WARNING"
		)
		...
		if (length(diagnostics$download_errors) > 0) {
			log_message(
				paste(
					"Download errors:",
					length(diagnostics$download_errors)
				),
				"extract_pdfs",
				"WARNING"
			)
		...
		if (length(diagnostics$extraction_errors) > 0) {
			...
			for (i in 1:min(
				...
				log_message(
					paste(
						"  -",
						diagnostics$extraction_errors[i]
					),
					"extract_pdfs"
				)
			}
	...
	metadata <- list(
		...
		download_success_rate = round(
			successful_downloads / nrow(tokens_data) * 100,
			1
		),
		extraction_success_rate = if (successful_downloads > 0) {
			round(
		...
		overall_success_rate = round(
			successful_extractions / nrow(tokens_data) * 100,
			1
		),
	...
	log_message(
		paste(
			...
			nrow(tokens_data),
			"NAPs",
			sprintf("(%.1f%%)", metadata$overall_success_rate)
		),
		"extract_pdfs"
	)
