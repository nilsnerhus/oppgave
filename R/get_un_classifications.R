...
get_un_classifications <- function() {
	...
	log_message(
		"Scraping UN geographic classifications from official sources",
		"get_un_classifications"
	)

	## --- Scrape SIDS list from UN OHRLLS -------------------------------------
	log_message(
		"Scraping SIDS list from UN OHRLLS",
		"get_un_classifications"
	)
	...
	log_message(
		"Scraping LLDC list from UN OHRLLS",
		"get_un_classifications"
	)
	...
	lldc_countries <- tryCatch(
		{
			...
			country_links <- rvest::html_nodes(
				page,
				"div.pane-content table a[href*='unctadstat.unctad.org']"
			)
...
my_total <- calculate_sum(5, 10)
another_call <- calculate_sum(3, 7)
third_call <- calculate_sum(1, 2)
